...
=head1 SYNOPSIS
    ...
    function SampleTest.testType()
        is_of_type(SampleTest.answer, number, "The answer is a number")
    end
...
A string with the date of last modification.
...
=item B<isnt_true>(I<expression>, I<explanation>)

The C<isnt_true()> function tests whether a certain expression evaluates to
...
=item B<isnt_equal>(I<value>, I<expected_value>, I<explanation>)

The C<isnt_equal()> function compares two values and tests whether these
...
=item B<does_match>(I<value>, I<pattern>, I<explanation>)

The C<does_match()> function tests whether a certain value matches a Lua
...
I<explanation> with a short description of the test.

=item B<doesnt_match>(I<value>, I<pattern>, I<explanation>)

The C<doesnt_match()> function tests whether a certain value does not match
a Lua pattern. If it does not, the function reports the test as B<passed>,
otherwise it reports it as B<failed>.

Replace I<value> with the value to test, I<pattern> with the pattern, and
I<explanation> with a short description of the test.

=item B<is_of_type>(I<value>, I<type>, I<explanation>)

The C<is_of_type()> function tests whether a certain value is of a
particular type. If it is, the function reports the test as B<passed>,
otherwise it reports it as B<failed>.

Replace I<value> with the value to test, I<type> with the expected data
type, and I<explanation> with a short description of the test.

=item B<isnt_of_type>(I<value>, I<type>, I<explanation>)

The C<isnt_of_type()> function tests whether a certain value is not of a
...
=item B<passed>(I<explanation>)

The C<passed()> function reports the test as B<passed>.

Replace I<explanation> with a short description of the test.

=item B<failed>(I<explanation>)

The C<failed()> function reports the test as B<failed>.
